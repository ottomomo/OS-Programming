...
int findFileByName(MyFileSystem *myFileSystem, char *fileName) {
	...
	for (i = 0; i < MAX_FILES_PER_DIRECTORY; i++) {
		if (myFileSystem->directory.files[i].freeFile == false) {
			if (strcmp(fileName, myFileSystem->directory.files[i].fileName)
					== 0)
				return i;
		}
	}
...
int findNodeByPos(int nodeNum) {
	int whichInodeBlock = 0;
	int whichInodeInBlock = 0;
	int inodeLocation = 0;

	whichInodeBlock = nodeNum / NODES_PER_BLOCK;
	whichInodeInBlock = nodeNum % NODES_PER_BLOCK;

	inodeLocation = (NODES_IDX + whichInodeBlock) * BLOCK_SIZE_BYTES
			+ whichInodeInBlock * sizeof(NodeStruct);
...
int initializeNodes(MyFileSystem *myFileSystem) {
	...
	for (; numNode < MAX_NODES; numNode++) {
		...
		// We alloc. an inode in mem. and copy the content from the backup file
			...
			if ((myFileSystem->nodes[numNode] = malloc(sizeof(NodeStruct)))
					== NULL) {
...
int myMkfs(MyFileSystem *myFileSystem, int diskSize, char *backupFileName) {
	...
	myFileSystem->fdVirtualDisk = open(backupFileName, O_CREAT | O_RDWR,
	S_IRUSR | S_IWUSR);
...
int readNode(MyFileSystem *myFileSystem, int nodeNum, NodeStruct* node) {
	...
	if (lseek(myFileSystem->fdVirtualDisk, posNode, SEEK_SET) == -1
			|| read(myFileSystem->fdVirtualDisk, node, sizeof(NodeStruct))
					!= sizeof(NodeStruct)) {
...
int reserveBlocksForNodes(MyFileSystem *myFileSystem, DISK_LBA blocks[],
		int numBlocks) {
	int i = 0;
	int currentBlock = 0;
...
int updateBitmap(MyFileSystem *myFileSystem) {
	if (lseek(myFileSystem->fdVirtualDisk, BLOCK_SIZE_BYTES * BITMAP_IDX,
	SEEK_SET) == (off_t) -1) {
		perror("Failed lseek in updateBitmap");
		return -1;
	}
	if (write(myFileSystem->fdVirtualDisk, myFileSystem->bitMap,
			sizeof(BIT) * NUM_BITS) == -1) {
...
int updateDirectory(MyFileSystem *myFileSystem) {
	if (lseek(myFileSystem->fdVirtualDisk, BLOCK_SIZE_BYTES * DIRECTORY_IDX,
	SEEK_SET) == (off_t) -1) {
		perror("Failed lseek in updateDirectory");
		return -1;
	}
	if (write(myFileSystem->fdVirtualDisk, &(myFileSystem->directory),
			sizeof(DirectoryStruct)) == -1) {
...
int updateSuperBlock(MyFileSystem *myFileSystem) {
	if (lseek(myFileSystem->fdVirtualDisk, BLOCK_SIZE_BYTES * SUPERBLOCK_IDX,
	SEEK_SET) == (off_t) -1) {
		perror("Failed lseek in updateSuperBlock");
		return -1;
	}
	if (write(myFileSystem->fdVirtualDisk, &(myFileSystem->superBlock),
			sizeof(SuperBlockStruct)) == -1) {
...
int myMount(MyFileSystem *myFileSystem, char *backupFileName) {
	...
	printf("SF: %s, %d B (%d B/block), %d blocks\n", backupFileName,
			...
			BLOCK_SIZE_BYTES, myFileSystem->superBlock.diskSizeInBlocks);
	printf("1 block for SUPERBLOCK (%u B)\n",
			(unsigned int) sizeof(SuperBlockStruct));
	printf("1 block for BITMAP, covering %u blocks, %u B\n",
			(unsigned int) NUM_BITS,
...
int readBlock(MyFileSystem *myFileSystem, DISK_LBA blockNumber, void *buffer) {
	...
	if (blockNumber < 0
			|| blockNumber >= myFileSystem->superBlock.diskSizeInBlocks) {
		fprintf(stderr, "Firts bocks to be read must be between 0 and %d\n",
				myFileSystem->superBlock.diskSizeInBlocks - 1);
		return -1;
	}

	if (lseek(myFileSystem->fdVirtualDisk, blockNumber * BLOCK_SIZE_BYTES,
	SEEK_SET) == (off_t) -1) {
		perror("Failed lseek in readBlock()");
		return -1;
	}
	if (read(myFileSystem->fdVirtualDisk, buffer,
...
int writeBlock(MyFileSystem *myFileSystem, DISK_LBA blockNumber, void *buffer) {
	...
	if (write(myFileSystem->fdVirtualDisk, buffer,
	BLOCK_SIZE_BYTES) != BLOCK_SIZE_BYTES) {
