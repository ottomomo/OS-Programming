...
int resizeNode(uint64_t idxNode, size_t newSize) {
	...
	int i = 0, diff = newSize - node->fileSize;
	...
	if (diff > 0) {
		...
		if (node->numBlocks && node->fileSize % BLOCK_SIZE_BYTES) {
			...
			int bytes2Write = (diff > (BLOCK_SIZE_BYTES - offBlock)) ?
			BLOCK_SIZE_BYTES - offBlock :
																		diff;
		...
		int newBlocks = (newSize + BLOCK_SIZE_BYTES - 1) / BLOCK_SIZE_BYTES
				- node->numBlocks;
		if (newBlocks) {
			memset(block, 0, sizeof(char) * BLOCK_SIZE_BYTES);
...
static int my_getattr(const char *path, struct stat *stbuf) {
	...
	if (strcmp(path, "/") == 0) {
		...
		stbuf->st_mtime = stbuf->st_ctime =
				myFileSystem.superBlock.creationTime;
		return 0;
	}
...
static int my_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
	...
	for (i = 0; i < MAX_FILES_PER_DIRECTORY; i++) {
		if (!(myFileSystem.directory.files[i].freeFile)) {
			if (filler(buf, myFileSystem.directory.files[i].fileName, NULL, 0)
					== 1)
				return -ENOMEM;
		}
	}
...
static int my_write(const char *path, const char *buf, size_t size,
	...
	fprintf(stderr,
			"--->>>my_write: path %s, size %zu, offset %jd, fh %"PRIu64"\n",
...
static int my_mknod(const char *path, mode_t mode, dev_t device) {
	...
	/// Update all the information in the backup file:
	int idxNodoI = -1, idxDir = -1;
	if ((idxNodoI = findFreeNode(&myFileSystem)) == -1 || (idxDir =
			findFreeFile(&myFileSystem)) == -1) {
		return -ENOSPC;
	}
	...
	reserveBlocksForNodes(&myFileSystem, myFileSystem.nodes[idxNodoI]->blocks,
			0);
 ...
 *
 */
static int my_read(const char * path, char * salida, size_t cantidad,
		off_t offset, struct fuse_file_info *fi) {

	//Bytes leidos
	int leidos = 0;
	//Bytes a leer
	int bytes2Read = cantidad;


	fprintf(stderr, "--->>>my_read: path %s amount %d offSet %d \n",
			path, bytes2Read, offset);


	//Nodo al que estamos apuntando
	NodeStruct *node = myFileSystem.nodes[fi->fh];

	//Buffer de lectura detamaño bloque ya que leemos de bloque en bloque
	//necesario para llamada a read del SO
	char buffer[BLOCK_SIZE_BYTES];

	while (bytes2Read) {
		int i = 0;
		int currentBlock = 0, offBlock = 0;
		//Calculamos el bloque al que queremos ir
		currentBlock = node->blocks[offset / BLOCK_SIZE_BYTES];
		// Calculamos la posicion del puntero de lectura en el bloque que toca
		offBlock = offset % BLOCK_SIZE_BYTES;

		//Leemos el bloque entero que nos toca
		if (readBlock(&myFileSystem, currentBlock, &buffer) == -1) {
			fprintf(stderr, "Error reading blocks in my_write\n");
			return -EIO;
		}

		//Ponemos en el buffer de salida los bytes que nos piden (a partir del offset)
		for (i = offBlock; (i < BLOCK_SIZE_BYTES) && (leidos < cantidad); i++) {
			salida[leidos++] = buffer[i];
		}

		//Calculamos el nuevo offset por si nos piden leer posiciones que no estan
		//solo en un bloque
		offset += i;
		//Quitamos los leidos de la cantidad a leer
		bytes2Read -= (i - offBlock);
	}
	sync();

	fprintf(stderr, "Correct read %d bytes\n", leidos);

	return leidos;
}

struct fuse_operations myFS_operations = { .getattr = my_getattr, // Obtain attributes from a file
