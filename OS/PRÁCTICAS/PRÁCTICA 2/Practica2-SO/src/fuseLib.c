...
static int my_unlink(const char *path)
{
	...
	//if(findFileByName(path, &idxNodoI))
	 ...
	 //Modificamos tamaño de Inodo a 0
	...
	// Liberamos el archivo del directorio raiz
...
{
	// buffer de salida, con tamaño bloque
	char buffer[BLOCK_SIZE_BYTES];
	// numero de B a leer,   leidos
	...
	while (bytes2Read) {
		int i = 0;
		int currentBlock, offBlock;
		//Bloque que queremos leer
		currentBlock = node->blocks[offset / BLOCK_SIZE_BYTES];
		// posicion en el bloque actual
		offBlock = offset % BLOCK_SIZE_BYTES;
		//lectura del bloque
		if (readBlock(&myFileSystem, currentBlock, &buffer) == -1) {
...
/*
static int my_link(const char *path1, const char *path2)
{
	int idxDir1=0, idxDir2=-1;
	fprintf(stderr, "--->>>my_Link: path1 %s, path2 %s \n", path1, path2);

	//if(!findFileByName(path1, &idxNodoI))
	 if((idxDir1 = findFileByName(&myFileSystem, (char *)path1 + 1)) != -1) {
	     return -ENOENT;
	 }

	 // id del Inodo de File1
	 	int idxNodoI = myFileSystem.directory.files[idxDir1].nodeIdx;

	 //if(findFileByName(path2, &idxNodoI))
	if ((idxDir2 = findFileByName(&myFileSystem, (char *) path2 + 1)) == -1) {
		return -ENOENT;
	}
	...
	// The directory exists
	if (findFileByName(&myFileSystem, (char *) path2 + 1) != -1)
		return -EEXIST;
...
struct fuse_operations myFS_operations = {
	...
	.read		= my_read,						// read a file already opened
//	.link		== my_link,						// create a link between 2 files
};
