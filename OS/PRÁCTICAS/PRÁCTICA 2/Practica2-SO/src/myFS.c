...
int readInodes(MyFileSystem* myFileSystem)
{
	...
	int i;
	for(i=0; i<MAX_NODES;i++){
		int posNode = findNodeByPos(i);
		if(lseek(myFileSystem->fdVirtualDisk,posNode,SEEK_SET) == (off_t) -1){
			perror("Failed lseek in readInodes");
			return -1;
		}
		if(read(myFileSystem->fdVirtualDisk,&nodoi,sizeof(NodeStruct))== -1){
		...
		if(!nodoi.freeNode){
			...
			copyNode(myFileSystem->nodes[i],&nodoi);
		}
...
int readBlock(MyFileSystem *myFileSystem, DISK_LBA blockNumber, void *buffer)
{
    ...
    if(blockNumber < 0 || blockNumber >= myFileSystem->superBlock.diskSizeInBlocks) {
        fprintf(stderr,"Firsts blocks to be read must be between 0 and %d\n", myFileSystem->superBlock.diskSizeInBlocks-1);
        return -1;
    }
