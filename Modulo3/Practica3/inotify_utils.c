...
extern int keep_running; // flag de ejecucion (sigue corriendo)
static int watched_items; //objetos vistos
int count_events;	// contador de eventos
extern char *diro;	//Directorio Origen
extern char *dird;	//Directorio Destino
...
void handle_event (queue_entry_t event)
{
  ...
  char *cur_event_filename = NULL; //nombre del fichero
  char *cur_event_file_or_dir = NULL; //tipo del evento
  /* This is the watch descriptor the event occurred on */
  int cur_event_wd = event->inot_ev.wd; //numero del descriptor de fichero del evento
  ...
  switch (event->inot_ev.mask & (IN_ALL_EVENTS | IN_UNMOUNT | IN_Q_OVERFLOW | IN_IGNORED))
    ...
    case IN_MODIFY:
      printf ("MODIFY: %s \"%s\" on WD #%i\n",
	      cur_event_file_or_dir, cur_event_filename, cur_event_wd);
      memset(patho, 0, sizeof patho);/*Limpia el array*/
      memset(pathd, 0, sizeof pathd);
    	strcpy(patho,  diro);
    	strcpy(file, cur_event_filename);
    	patho[strlen(patho)]='/';
    	strcat(patho, file);
    	...
    	fo= open(pathd, O_CREAT | O_RDWR | O_TRUNC);
    	fd =open(patho, O_RDONLY);
    	copia(fd,fo);
    	printf("Modificando en el destino:%s\n",pathd);
      break;
    ...
    case IN_ATTRIB:
      ...
      memset(pathd, 0, sizeof pathd);
    	...
    	{
    		printf("\nError en stat %s\n",patho);
    		perror("stat permisos");
    	} 
    	else
    	{
    	    if ((chmod(pathd, st.st_mode & 07777) == -1))
    	    {
    		perror("chmod");
    	    }
    		printf("Cambio de permisos en el destino:%s\n",pathd);
    	}
    break;
    ...
    case IN_DELETE:
      ...
      memset(pathd, 0, sizeof pathd);
    	strcpy(pathd,  dird);
    	pathd[strlen(pathd)]='/';
    	strcat(pathd, cur_event_filename);
    	printf("Borrando en el destino:%s\n",pathd);
    	if((unlink(pathd)) == -1)
    		printf("Archivo no existia en le destino");
    break;
    ...
    case IN_CREATE:
      ...
      memset(pathd, 0, sizeof pathd);
    	...
    	if(cur_event_file_or_dir == "File"){
    	  fo = open(pathd, O_CREAT | O_RDWR,buf.st_mode);
    	  fd = open(patho, O_RDONLY);
    	  copia(fd,fo);
    	}else{
    	  fd = mkdir(patho, O_RDONLY);
    	}
    	printf("Creando en el destino:%s\n",pathd);
...
/******AMPLIACION USO DE POLL***********/
  struct pollfd fdset[1];
  fdset[0].fd = fd;
  fdset[0].events = POLLIN;

  return poll(fdset,1,-1);
}

int process_inotify_events (queue_t q, int fd)
{
  //while (keep_running && (watched_items > 0))
  if(keep_running && (watched_items > 0))
  {
